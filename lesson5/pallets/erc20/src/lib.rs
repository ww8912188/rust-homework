...
decl_storage! {
	trait Store for Module<T: Trait> as Erc20{
			...
			Tokens get(fn token_details):  map hasher(blake2_128_concat) u32 => Erc20Token;
			Balanceof get(fn balance_of): map hasher(blake2_128_concat) (u32, T::AccountId) => u64;
			// TotalSupply get(fn token_total_supply): u64 = 21000000;
...
decl_module! {
		pub struct Module<T: Trait> for enum Call where origin: T::Origin {
				...
				fn init(origin, name: Vec<u8>, ticker: Vec<u8>, total_supply: u64) -> DispatchResult {
						...
						Self::deposit_event(RawEvent::Issue(token_id, sender.clone(), total_supply));
						Ok(())
				}

				// transfer tokens from one account to another
				#[weight=0]
				fn transfer(_origin, token_id: u32, to: T::AccountId, value: u64) -> DispatchResult {
...
impl<T: Trait> Module<T> {
	fn _transfer(token_id: u32, from: T::AccountId, to: T::AccountId, value: u64) -> DispatchResult {
